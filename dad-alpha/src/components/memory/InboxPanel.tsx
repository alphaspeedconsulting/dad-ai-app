...
export function InboxPanel() {
  ...
  const handleCapture = async () => {
    ...
    if (householdId) {
      api.sharedInbox.create(householdId, { content: trimmed }).then(() => {
        updateInbox(item.id, { content: item.content });
      }).catch(() => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="dad-input flex-1 text-alphaai-sm"
        />
        <button
          onClick={handleCapture}
          disabled={!newTask.trim()}
          className="w-11 h-11 dad-gradient-hero rounded-xl flex items-center justify-center flex-shrink-0 disabled:opacity-40"
        >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center flex-wrap gap-2 mt-2">
                    ...
                    {item.assigned_to_name && (
                      <span className="dad-chip-secondary text-alphaai-3xs px-2 py-0.5 flex items-center gap-1">
