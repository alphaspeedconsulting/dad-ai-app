...
export function ReferralBanner() {
  const [dismissed, setDismissed] = useState(() => {
    ...
    return localStorage.getItem("dad-referral-banner-dismissed") === "true";
  });
  ...
  return (
    <Link href="/referral" className="block">
      <div className="dad-card p-4 flex items-center gap-3 bg-brand-glow/5 border border-brand/10 hover:bg-brand-glow/10 transition-colors relative">
        ...
        <button
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            localStorage.setItem("dad-referral-banner-dismissed", "true");
            setDismissed(true);
          }}
