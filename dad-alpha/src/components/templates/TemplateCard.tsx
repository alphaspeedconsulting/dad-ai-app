...
export function TemplateCard({ template }: TemplateCardProps) {
  return (
    <Link
      ...
      className="dad-card p-4 hover:bg-surface-container-low transition-colors block"
    >
      ...
      {template.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {template.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="dad-chip text-alphaai-3xs px-2 py-0.5">
              {tag}
            </span>
          ))}
          <span className="dad-chip-secondary text-alphaai-3xs px-2 py-0.5">
