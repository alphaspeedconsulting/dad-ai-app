...
export default function CaregiversPage() {
  ...
  const handleRemove = async (id: string) => {
    if (!householdId) return;
    try {
      await api.caregivers.revoke(householdId, id);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-lg mx-auto px-4 pt-24 pb-24 space-y-4">
        {/* Info card */}
        <div className="dad-card p-4 bg-brand-glow/5 border border-brand/10">
        ...
        {isLoading ? (
          ...
          caregivers.map((cg) => (
            <div key={cg.id} className="dad-card p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {cg.permissions.map((perm) => (
                  <span key={perm} className="dad-chip text-alphaai-3xs px-2 py-0.5">
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-surface rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-6 space-y-5">
        ...
        <button
          ...
          className="dad-btn-primary w-full disabled:opacity-50"
        >
