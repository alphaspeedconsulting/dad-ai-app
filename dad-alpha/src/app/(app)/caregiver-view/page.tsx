...
export default function CaregiverViewPage() {
  ...
  useEffect(() => {
    ...
    api.caregivers
      .view(accessToken)
      .then(setData)
      .catch((err: unknown) => setError(err instanceof Error ? err.message : "Access denied or link expired"))
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 dad-gradient-hero rounded-xl flex items-center justify-center mx-auto mb-4">
            <span className="material-symbols-outlined text-[24px] text-on-primary">handyman</span>
          </div>
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="dad-card p-8 text-center max-w-sm w-full">
          ...
          <Link href="/login?mode=signup" className="dad-btn-primary">
            Join Dad.alpha
          </Link>
        </div>
      </div>
      ...
      <header className="bg-background border-b border-border-subtle/10">
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 dad-gradient-hero rounded-lg flex items-center justify-center">
              <span className="material-symbols-outlined text-[16px] text-on-primary">handyman</span>
            </div>
            <div>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-5">
          ...
          <section className="dad-card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {data.family_members.map((m, i) => (
                <span key={i} className="dad-chip text-alphaai-xs">
          ...
          <section className="dad-card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {data.allergies.map((allergy, i) => (
                <span key={i} className="dad-chip-secondary text-alphaai-xs px-3 py-1">
                  {allergy}
                </span>
              ))}
            </div>
        ...
        <div className="text-center py-4">
          <div className="flex items-center justify-center gap-1.5">
            <div className="w-5 h-5 dad-gradient-hero rounded-md flex items-center justify-center">
              <span className="material-symbols-outlined text-[10px] text-on-primary">handyman</span>
            </div>
            <span className="text-alphaai-3xs text-muted-foreground">
              Powered by Dad.alpha
            </span>
          </div>
        </div>
      </main>
