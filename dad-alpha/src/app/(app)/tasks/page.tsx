  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-lg mx-auto px-4 pt-20 pb-4 space-y-6">
        {/* Hero summary */}
        <div className="dad-gradient-hero rounded-2xl p-6 text-on-primary">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <Link
              key={action.label}
              href={action.href}
              className="dad-card p-4 flex items-center gap-3 hover:bg-surface-container-low transition-colors"
            >
        ...
        {/* Tip */}
        <div className="flex items-start gap-3 p-4 rounded-xl bg-tertiary-container/20 border border-tertiary/10">
          ...
          <p className="text-alphaai-xs text-muted-foreground">
            <strong className="text-foreground">Dad Tip:</strong> Tasks update in real-time.
  ...
  return (
    <div className="dad-card p-4 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <span className="dad-chip-secondary text-alphaai-3xs py-0.5 px-2 uppercase">
...
function StepDot({ step }: { step: TaskStep }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-1">
      <span className={`w-2.5 h-2.5 rounded-full ${colors[step.status] ?? "bg-surface-container"}`} />
      <div className={`flex-1 h-0.5 ${step.status === "completed" ? "bg-brand" : "bg-surface-container"}`} />
    </div>
  );
}
...
function CompletedTaskCard({
  ...
  return (
    <div className="dad-card p-4 bg-surface-container-low/50 flex items-center gap-3">
