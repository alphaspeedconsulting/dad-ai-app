  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-32">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 dad-gradient-hero rounded-2xl flex items-center justify-center">
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="dad-input pl-10"
        />
      </div>

      {/* Filter tabs */}
      <div className="relative mb-4">
      <div className="flex gap-2 overflow-x-auto pb-3 dad-no-scrollbar pr-8">
        {FILTER_TABS.map((tab) => (
          <button
            key={tab.key}
      ...
      {!showAddForm && (
        <button
          ...
          className="w-full dad-card p-4 mb-4 flex items-center gap-3 text-left hover:bg-surface-active/50 transition-colors"
        >
      ...
      {/* Add form */}
      {showAddForm && (
        <div className="dad-card p-4 mb-4 space-y-3">
          <textarea
            value={newContent}
            onChange={(e) => setNewContent(e.target.value)}
            placeholder="What do you want to remember?"
            rows={3}
            className="dad-input resize-none"
            autoFocus
          />

          <div className="flex gap-2 overflow-x-auto dad-no-scrollbar">
          ...
          <input
            ...
            className="dad-input text-alphaai-xs"
          />
          ...
          <div className="flex gap-2 justify-end">
            <button
              ...
              className="dad-btn-outline px-4 py-2 text-alphaai-xs"
            >
              Cancel
            </button>
            <button
              ...
              className="dad-btn-primary px-4 py-2 text-alphaai-xs disabled:opacity-40"
            >
              Save
            </button>
          </div>
      ...
      {!isLoaded ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="dad-card p-4 h-24 dad-skeleton" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((item) => (
            <div key={item.id} className="dad-card p-4">
              {editingId === item.id ? (
                <div className="space-y-2">
                  <textarea
                    value={editContent}
                    onChange={(e) => setEditContent(e.target.value)}
                    className="dad-input resize-none text-alphaai-sm"
                    rows={3}
                    autoFocus
                  />
                  <div className="flex gap-2 justify-end">
                    ...
                    <button
                      ...
                      className="dad-btn-primary px-3 py-1.5 text-alphaai-3xs"
                    >
                      Save
                    </button>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        <span className="dad-chip text-alphaai-3xs px-2 py-0.5">
                          {CATEGORY_CONFIG[item.category]?.label ?? item.category}
                        </span>
                        {item.tags.map((tag) => (
                          <span
                            key={tag}
                            className="dad-chip-secondary text-alphaai-3xs px-2 py-0.5"
                          >
                            {tag}
                          </span>
                        ))}
